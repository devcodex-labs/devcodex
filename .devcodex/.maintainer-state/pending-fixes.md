...
## Pending Fix #PF-009
- 文件：`instructions/18-spec-radar.instructions.md` §与其他文件的关系 L201
- 问题：`data/process-improvements.md` 描述为"PF 修复后的改进记录（PI 条目）"，但实际写入触发条件（`01-common C17`）是"用户建议的执行策略经 AI 确认更优时"——两者完全不同。当前描述误导 AI 认为 PI 条目是 PF 修复流程的产物，而非独立的用户策略记录机制。
- 建议修复：将描述改为"用户建议策略被 AI 接受时自动追加 PI 条目（触发：C17）"，或新增一行同时说明两种触发场景（若 PF 修复后确实也应写 PI）。属 self-fix dev 流程（表述/含义变更）。
- 状态：已关闭（v1.5.2，18-spec-radar §与其他文件的关系 描述已更正为 C17 触发条件）
