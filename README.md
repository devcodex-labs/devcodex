...
├── skills/         ← Skill 详细检查标准（48 个，按需读取，含默认分析、用户文档、复审清单、自我进化治理、README 专项能力、spec-governance 与 5 个支撑型 Skill）
...
├── skills/        # Skill 详细检查标准（48 个，按 01-common §按需读取表 路由读取）
├── prompts/       # Prompt 模板（30 个）
...
规范治理新增 `spec-governance` Skill：记录类动作先做意图识别，再由 RecordRouter 分流到 `violations / pending-fixes / process-improvements（优化清单，PI） / pending-issues / gap-registry`；所有模式下每条用户消息还会执行主动 Improvement Intake，把已验证更优且可泛化的策略记录到优化清单，并在暴露规范缺口时同步联动 PF。规范/控制面/路径/模板/部署/校验链变更后必须执行 SCV（Spec Change Verification），避免修复一处后引入漂移。`GovernanceGateRegistry` 统一给 PC4、技术方案、实施计划、报告模板和 validate 探针提供 `gateGroup / ownerSkill / trigger / validationRoute` 索引，避免把完整 Gate 长清单继续复制到通用 instructions 或 prompts。
