...
本版本完成长任务 **墙钟预算 / 外部等待会计 / 长任务授权门禁**（PI-118/PF-137），交付面 **WorkspaceSyncStatus** 与 **CompletionEvidenceGate**，条件 **PostDeliverySelfCheck**，并收口清单 11 后续工程与 validate 全绿。

## 本批未发布增量（归档自 unreleased）
...
- **Batch ζ 长任务预算与完成证据（全量收口）**：`execution-contract` 落地 `ExecutionBudgetGate` / `ExternalWaitAccountingGate` / `LongTaskAuthorizationGate`（PI-118/PF-137：墙钟执行预算、外部等待分列、续跑=新 cycle+授权）；`memory`/`report`/`compliance` 增加 budget 锚点、`WorkspaceSyncStatus`、`CompletionEvidenceGate`、条件 `PostDeliverySelfCheck`（T11~T13）；`C08` 交叉引用长任务预算；`instructions` CrossProject 索引 `long-task-budget`；台账回写 PI-109/118、PF-129/137 与 v1.11.25/26 已吸纳表项 closed。
- **Batch ε 工程修复（@rocky）**：`skills/*/**` 打包排除根级 `portfolio.json` / `portfolio-evidence.json`（~600KB 不进 npm pack）；`test:fast`（无 website build）与 `test:full`；Claude 权限收紧（去 bare `Bash`、`enableAllProjectMcpServers=false`、ask 合并 bare Bash）；Codex hook 改为 monorepo parent-walk（与 Claude 对齐，同步 `codex/hooks.json`）；Copilot/Claude 安装内容相等跳过（content-equal skip + outdated 提示）；规范 banner 对齐 package `1.14.0`；Cursor 矩阵标注 best-effort。
- **Batch γ 分析能力（@rocky）**：新建 active Skill `incremental-project-analysis`（ABS-01～05：知识快照、选择性失效、BatchProgress 强制交付、GlobalBacklog、精度合同、双层验证）；注册 plugin + portfolio 78（75 active+3 gray）；接通 `analyze-default` / `skill-gap-analysis` / `13-analyze`；同步 README/website 计数与 V92/V97 冻结值。
- **Batch β 上下文预算（@rocky）**：`profile_load` 硬预算——无 `files` 须 `explicitFull+fullReadReason`，否则 `PROFILE_LOAD_BUDGET`；支持 `maxFiles`/`maxBytes` 截断与 deferred 清单；`load-profile` Skill 同步；MCP 测试与 hooks bootstrap 夹具已改。
- **Batch α 正确性（@rocky）**：PC0/PC7 三面同步为 `ContextReadPlan`+回执（`17-compliance` / `precheck-status` / `instructions`）；fix CP 图改为触发时 `[CP3]→执行`；C19 对齐 `PostConfirmationReviewScopeGate`；memory/report/compliance 写入 NewSessionContinuationCard、FindingThemeCoverageMatrix、SessionTimingCard；website `compliance-framework` 纠正历史 SC/FC 语义冲突。
- **Batch 0 HOST-grok**：正式 agent 枚举新增 `grok`（MCP `agent-identity.cjs`、memory/profile 服务器、validate-profile、15-memory / memory Skill / instructions、Hook/CLI 宿主探测）。运行时 `detectRuntimeAgent` **禁止**在无法识别时默认 `claude-code`；支持 `DEVCODEX_AGENT=grok` 与 `GROK_AGENT` 等信号。历史 `codex/` 产物不迁移。补充 MCP 测试（identity 模块 + grok 写入）。关联 ABS-16 / HOST-01/02 / PI 确认包 11。
...
- `node scripts/validate.js` → All checks passed
- 工作区根 `node index.js update` 三宿主同步
- 台账 PI-118/109、PF-137/129 与 already-fixed 表项回写
