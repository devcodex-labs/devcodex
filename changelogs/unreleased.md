...
## 当前未发布实现候选
- **工作区 Skill 意图结构化与渐进路由**：83 个 active Skill 增加 `SkillIntentV1`；运行时构建 W + managed G 动态快照，经单一 `skill_route` MCP Tool 执行 catalog/commit/load_stage/status、0/1 自由选择、依赖闭包、冲突与预算校验、分阶段正文加载、条件重规划、幂等重放、上下文截断恢复和有锁 GC。仅有新鲜能力证据的 exact `grok-cli-single/global-launcher-local-stdio` 可进入 unified；其他宿主 fail-closed 到 legacy。五精确宿主身份、Copilot camelCase Hook、直接 S15 与 `SkillRouteRetirementGateV1` 已落地；legacy 只有在当前 runtime/adapter digest 下连续 72 小时、每宿主 20 次、总计 100 次且六类异常全为 0 后才可退役。当前真实检查 accepted=1/100，因此 legacy 保留。MCP 为宿主按需启动的本地 stdio 子进程，无端口、HTTP、远程服务或常驻 daemon。W 在每轮动态发现；用户原生 `AGENTS.md` / `CLAUDE.md` / `SKILL.md` 与个人 Skill 保持宿主所有权，同名不构成清理或覆盖权限。完整 full/staged `87/87`、profile-deploy、package-release 与真实 tarball global-install 均通过。
