...
- **Dialogue-Primary Closeout B1**：新增 `classifyDialogueNarrativeSample` / `hasReadableNarrativeSnippet`（完成态/分析收敛须对话内可读叙事）；`classifyAnalysisArtifactDeliverySample` 对「仅报告链接」返回 `link-only-thin`；同步 user-visible-output-contract、report、13-analyze、report-analysis prompt、V102 锚点与单测。lifecycle 接线留 B2。需求：`完成态对话内可读收口` CP2。
