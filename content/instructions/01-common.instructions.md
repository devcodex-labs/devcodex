...
| C08 | Token / 长任务预算防护 | 超 10 轮关注上下文；超 13 轮保存编码检查点；超 15 轮或多文件长任务分批保存完整记忆和恢复线索。内部轮数、读取预算或 compaction 只产生提示和分批/重取动作，不取消用户已授权的原任务，不要求为继续工作反复确认。`execution-contract` 记录真实执行与外部等待用时；用户明确设置的资源上限仍须遵守，消耗不得静默清零 | — |
...
| S07（入口检查强制）| 🔴 致命自修正（`instruction-fallback` 模式自检触发，自动补输出 PC0~PC10 基础状态）| 🔴 致命自修正（`instruction-fallback` 模式自检触发，自动补输出 PC0~PC10 + dev 扩展诊断）|
...
- 每次写入由 `MutationFootprintV2 → ArtifactSlotDecisionV2 → TaskOwnedMutationLeaseV2 → TaskRecoveryStoreV5 prewrite → MutationObservationReceiptV1` 单次闭环；0-target、unknown、partial、越界或 required effect 未发生必须 `needs-reconcile`。任务准入对同一请求精确 readback 最大完整阶段前缀并幂等补齐阶段内缺失文件；artifact closeout 由 `memory_artifact_mutation_reconcile_v1` 绑定当前 ingress/formal route/operation/CAS 与实际效果收口，partial/零效果只能从 V5 预写的完整 pre-observation 重观察，且不签发 mutation authority。reserve 只能恢复主槽仍在等待且 operation/decision/lease/footprint 全部一致的写入；reconciled projection 必须保留后继验证所需的 recovered effect 集合。terminal 四证据 closeout 后立即解绑 route/owner，只有显式 reopen 才可获得新 owner generation。
- Server-owned MCP state operation 不得被 host artifact mutation 层重复接管：`MemoryFileTransactionV1` 四类 writer 保留 ingress/route/PC 时序检查但不产生通用文件 closeout，task/operational/reconciliation authority-control Tool 不自消费其签发的 mutation authority；所有已发布 MCP qualifier 先归一到 canonical leaf，第三方同名 Tool 保持未知且不可豁免。
