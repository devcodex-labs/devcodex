...
- **CapabilitySurfaceDecisionGate**：新增或改变 Rule/Skill、Prompt、MCP Resource/Resource Template/Tool、Task 增强 Tool、CLI 或 Hook 时，先由 `spec-governance` 单写中央 `CapabilitySurfaceDecisionV1`；领域 Skill 只提供证据和保存 `decisionRef`，消费者不得复制中央判定字段。缺 decision、identity stale/blocked 或无直接宿主/协议证据时不得创建或激活能力资产。
