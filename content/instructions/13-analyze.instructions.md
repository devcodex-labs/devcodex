...
### 对话内可读收口（Dialogue-Primary）

- analyze / 方案审阅收敛时，用户面必须是**可读摘要 + 报告链接**；禁止仅报告链接或「详见报告」宣称完成（`link-only-thin` / `classifyDialogueNarrativeSample`）。
- 长论证进 `reports/**`；对话内至少：结果/结论一句 + ≥1 实质要点 + 推荐结论（如适用）。
- 默认不要引导用户打开预览软件阅读报告；路径供归档与深读。
