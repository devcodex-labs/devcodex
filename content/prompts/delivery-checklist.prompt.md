...
7. 持久化投影继续使用 `LinkCapabilityDecisionV1`；用户面通过 `ArtifactDeliveryResolver` 按 presentationSurface + capability evidence 生成 `HostLinkCapabilityDecisionV2`，并为每个目标生成 `ArtifactDeliveryAttemptV1`；只有 action 与 readback 均成功才写 opened，renderer-only/失败/不可回读必须绝对路径 fallback；
