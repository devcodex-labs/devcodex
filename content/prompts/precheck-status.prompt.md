...
| 项 | 内容（人话；禁止进度缩写） |
|----|----------------------------|
| PC0 | 版本：已安装 package version · 活动 runtime generation · 可选源码候选 `{packageVersion, shortHead, dirty}` · alignment |
...
- 输出语言：使用 `LanguageContextV2` 的任务主语言；yes/no、确认码、路径、版本、代码和引用文本不得切换任务主语言
...
- PC0：必须区分已安装包、当前进程加载的 runtime generation 与可选源码候选；只有可证明同一构建身份时才写 `aligned`，仅版本相同写 `version-only`，源码 dirty/版本领先写 `source-ahead`，运行时版本冲突写 `runtime-mismatch`，证据不足写 `unverified`
...
- 先用 `user-visible-output-contract` 形成完整 `EntryCheckModelV3` 和 Envelope，再渲染为 rich/portable/plain；状态词固定为 PASS/WARN/BLOCK/UNVERIFIED/N/A
