...
- 用户可见输出契约变化时，把 `ArtifactDeliveryManifestV1 → UserFacingArtifactSetV1 → PostCompletionActionSetV1 → EntryCheckModelV3 → DevCodexVisibleEnvelopeV3 → ArtifactDeliveryResolver → HostLinkCapabilityDecisionV2 + ArtifactDeliveryAttemptV1` 同步到 instructions、report/memory/compliance/TestRoute、precheck/progress/delivery/report prompts、visible Hook、README、website、Profile、plugin/portfolio、package 与 validate probes；`DevCodexVisibleEnvelopeV1/V2` 与 `LinkCapabilityDecisionV1` 保留给兼容读取。历史归档可保留旧表述，但当前消费者不得按宿主名、renderer label 推定可点击/已打开，强制单一路径表示或把 internal-only 文件默认列给用户。
