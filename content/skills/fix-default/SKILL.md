...
模型约束必须容忍表达变化：以原任务目标、真实授权范围和实际工具效果核验行为；同义标题、缺字段、陈旧上下文或登记失败优先自动补正、重取或分批续办，不依赖 100% 文本匹配。宿主拒绝、真实写者冲突和未知副作用只影响对应操作，保留待恢复内容并继续不受影响的工作。
...
- 多个修复项、并行修复、子 Agent、子会话或 worktree 进入执行前，必须先调用 `requirement-parallel-orchestration`；valid `ParallelLaunchCardV1` 仍需当前宿主 direct capability、parallel-eligible `HostSubagentDispatchPlanV1` 与 `AgentWorkLeaseV1` 才可派发。child 不得直接写 root checkout 或正式共享状态；timeout/cancel/late/路径或 dirty 漂移一律 quarantine，根 Agent 自动接管相同 work item 串行修复。任一证据缺失时源码写入保持串行，不能因无并行能力阻断修复任务。
...
- 最小实现守门：CP1 问题确认必须给出 `WorkflowPlanDecisionV1`；修复默认 `designDepth=minimal`，只解决已确认根因和影响范围。流程仪式、方案深度、验证等级独立，用户当前明确意图优先；禁止无计划新增抽象、通用配置、预留扩展点或未确认防御分支，确需超出范围或升级任一轴时先重算结构化意图并回 CP2/CP3，只有 executionDecision=confirm 才等待。旧复杂度字段只读兼容，不得新写入
