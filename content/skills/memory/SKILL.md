...
- 原子准入取得的 owner 在 CP pending 时不授予 mutation authority。CP confirmation 成功后，若 owner receipt 尚未观察到当前 CP，使用 `memory_task_write_owner renew` 对 `FencedTaskWriteOwnerLeaseV2` 做 generation/nonce/CAS 与 CP readback；正式 mutation 只有 finalized admission + exact CP + active owner 才可继续。handoff/takeover/reopen 必须形成新 transition receipt，旧 nonce 永不恢复 authority。
...
- artifact mutation 因工具返回失败、authority closeout 漂移、partial observation 或零效果进入 `needs-reconcile` 时，调用 `memory_artifact_mutation_reconcile_v1`。完整 observation 直接复证 effect；partial/零效果必须消费 V5 预写保存的 `ArtifactMutationReconciliationInputV1`，从完整 pre-observation 对同一 footprint 重新观察。Tool 还必须复证当前 server-owned ingress、同 session formal route、operation/closeout CAS、primary 或 emergency reserve 来源，以及普通文件系统快照；只生成 `ArtifactMutationReconciliationReceiptV1` 并关闭既有 pending 状态，不执行文件 mutation、不授予权限或 mutation authority。reserve 仅在主槽仍保留同一 operation、decision、lease 与 footprint 时可采用；压缩 projection 必须保留 recovered effect 集合供后继消费者验证。
