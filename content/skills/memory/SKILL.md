...
- 正式任务终态调用 `memory_task_terminal_v1`，精确提交当前 write context 的 lifecycle revision、state sequence、writer generation 与 settled-set digest，并以 ECR/report/memory/completion 四个互不复用的证据 identity 写 terminal receipt；未结算/待 reconcile operation、stale fence 或证据漂移都必须零终态提交，相同 replay 零新写。成功后 route/owner 立即解绑。Stop/PreCompact 仅 checkpoint，不等价 terminal。
- 每次正式 artifact mutation 必须先持久化唯一未结算的 `TaskOperationRecordV1`：prepare 绑定 idempotency key、writer generation、expected state、exact targets 与 before digest，dispatch 后 effect unknown 禁止自动重试，observe 后只能 settle 或进入 reconcile-required。工具返回失败、authority closeout 漂移、partial observation 或零效果进入 `needs-reconcile` 时，调用 `memory_artifact_mutation_reconcile_v1`。完整 observation 直接复证 effect；partial/零效果必须消费 V5 预写保存的 `ArtifactMutationReconciliationInputV1`，从完整 pre-observation 对同一 footprint 重新观察。Tool 还必须复证当前 server-owned ingress、同 session formal route、同一 operation record/closeout CAS、primary 或 emergency reserve 来源，以及普通文件系统快照；只生成 `ArtifactMutationReconciliationReceiptV1` 并关闭既有 pending 状态，不执行文件 mutation、不授予权限或 mutation authority。reserve 仅可把主槽中同一 dispatched/observed record 推进到 reconcile-required，并继续匹配 decision、lease 与 footprint；压缩 projection 必须保留 recovered effect 集合供后继消费者验证。
...
- `memory_session_write` 接受已有 allocation receipt 的 `sessionId` + `sessionBinding`，也可从同一宿主/任务的实际分配记录自动恢复省略字段；同宿主/任务/日期重复分配应幂等。writer 用 `MemorySessionWriteReceiptV1` 证明目标段及非目标段隔离和实际读回。真实错配、歧义或无法写入时保留内容与失败事实，继续原任务并在可恢复入口补写；不得虚构默认编号或绑定。旧版无 binding marker 的 daily 会话保持可读，需要续接时自动分配新的绑定会话。

`memory_summary_append` 优先传结构化 `entry`（summary、实际 status，及可选 type/sessionId/date），由运行时构造表格行、恢复当前会话并转义内容；旧 `row` 仍兼容。类型只从明确输入或已观察工作流恢复，状态必须如实提供，不补造 `01/active/other`。模板质量与登记失败不构成任务继续的前置门禁。
...
- 上述四类 writer 是 server-owned 单一事务 Owner：Hook 仍校验实际指令、项目/session route 与 PC0～PC10 产物时序，但不得再把其逻辑 URI 套入通用文件 `TaskOwnedMutationLeaseV2 / needs-reconcile`。`memory_workflow_operational_write_lease` 等 authority-control Tool 只描述后继权限并由服务端核验 ingress，也不得自消费 artifact mutation authority。直接名、`mcp__devcodex_memory__*`、`mcp__devcodex-memory__*`、`devcodex-memory/*` 与 `devcodex-memory__*` 必须归一为同一 leaf；未知第三方同名 Tool 不能借此豁免。
