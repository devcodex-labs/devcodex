...
Grok **UserPromptSubmit** cannot inject model context (stdout ignored). Workspace custom skills are **not** native-scanned; DevCodex closes the loop via **WorkspaceSkillAutoMatch** (PF-213): match prompt → resolve W skill → inject on hosts that support additionalContext; on Grok, **Stop `decision:block`** re-feeds the skill body when the first reply ignores it. The plugin is user-global and does not own a workspace. Resolve the nearest `workspace-namespace` marker from cwd, keep task state under that workspace's `.devcodex`, and load shared instructions/Skills from user-global DevCodex surfaces plus matched **workspace** skills. Full-evidence route: `devcodex grok`.
...
|------|-------|------------|----------------|
| **Full** | `devcodex grok` (official `--rules` kernel bind) | Controlling `AGENTS.md` in context; PreToolUse hard deny uses Grok `decision:deny`; context acquisition is **path-observable**; **conditional** Stop hard-continue (`decision:block` when `lastAssistantMessage` present — skill auto-match / completion gate) | UserPromptSubmit context inject; unconditional Stop without body / after softCap fail-open; verified PC0 from Hook payload alone |
...
4. Follow the kernel's intent-first route and read only selected files from `~/.agents/skills/` plus the resulting workspace Profile/context plan, **and any WorkspaceSkillAutoMatch hit under `.devcodex/workspace/skills/`**.
