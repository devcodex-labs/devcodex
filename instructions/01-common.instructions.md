...
| 影响点 | `prod`（默认）| `dev` |
|--------|:------------:|:-----:|
| CP 门控 | 🔴 强制等待用户确认 | 建议性（输出但不阻断等待）|
| 合规检查 | 全量 FC1~FC6 + SC1~SC13 + RC1~RC4 + T1~T9 | **仅 FC4/FC5** |
| 预检查输出 | 不输出预检查状态块 | 输出 PC1(Token轮次)/PC2(待跟进)/PC3(未完成任务) |
...
- 三问全指向分析 → `analyze` vs `audit`（按覆盖范围和收敛期望区分）
