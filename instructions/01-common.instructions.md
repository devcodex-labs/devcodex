...
| C19 | 确认后前置复审 | 每次用户明确确认后、进入下一阶段前，必须执行 `PostConfirmationReviewScopeGate` 并输出 **ReviewGradeCard**：映射 **轻量=R1**（须 `skipReason`）、**标准=R2**（默认）、**全面=R3**（高风险+冻结清单）、**发布安全=R4**；命中控制面 / 多文件联动 / 真相源同步 / 模板-示例-校验链须追加交叉验证；阻断项先修正并重确认。ECR 默认 R2，禁止「永远轻量」口径 |
