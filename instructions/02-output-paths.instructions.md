...
│   ├── 03-实施方案/                  # ⚠️ 条件（多子模块/多阶段实施时）
│   │   └── *.md                     #   各子模块/阶段实施细节，不含时间线
│   ├── 04-实施计划.md               # 🔴 强制
│   ├── 05-实施进度.md               # 🔴 强制（任务跨 2 轮以上会话时）
│   ├── scripts/                     # ⚠️ 条件（有辅助脚本时）
│   │   └── *.js / *.sh              #   接口验证/数据迁移/数据填充等辅助脚本
│   ├── *-接口验证.http              # 🔴 强制（有接口变更时）
│   ├── *-接口验证.cjs               # 🔴 强制（有接口变更时）
...
| **scripts/ 触发条件** | 需求任务中有辅助脚本（接口验证/数据迁移/数据填充等）时创建 `requirements/<任务>/scripts/`；禁止放入业务逻辑或网络请求 |
| **禁止写入源码目录** | 脚本/测试/辅助文件严禁放入项目源码目录 |
| **强制产物首轮完成** | 01/02/04 在首轮会话结束前必须创建；03 按条件触发 |
