...
| **强制产物首轮完成** | 默认 01/04 在首轮会话结束前必须创建；其中 `04-实施计划.md` 可按轻计划摘要 / 完整实施计划两档落地；SimpleTaskFastPath 命中时 01/04 允许 `N/A + skipReason`，但 PC0~PC7、Profile、报告、记忆、安全底线和必要验证仍不可省略；02-技术方案.md 和 实施方案/ 按条件触发；services/ 在 CP2 后按需创建；强触发条件命中时 `05-实施进度.md` 必须在执行前初始化 |
