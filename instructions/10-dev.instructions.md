...
- **PackageNameAuthorityGate**：npm/GitHub Packages、插件、bin、exports、scope、安装说明或发布验证变更必须核对 `package.json`、`plugin.json`、registry/包管理器证据和官方命名约束；禁止凭目录名或历史记忆推断包名。
- **PerformanceBenchmarkFirstGate**：涉及“最快 / 第一 / 优于 / 性能提升 / 压测 / benchmark”时，CP1/CP2 必须冻结基线、环境、版本、指标、负载、比较对象和成功阈值；缺少基线只能给条件结论。
- **PublicModuleDifferentiationGate**：公开模块、SDK、CLI、插件、文档站能力或对外 API 变更必须区分 public API、内部 helper、示例代码、发布包文件、消费者入口和历史镜像；正式文档只承诺真实公开面。
- **V2MCPFirstPlanningGate**：DevCodex v2 一期规划必须以 Intent-Gated Hosted Spec MCP、Codex-only 可验证 MVP、私有可追踪 docs 与无本地规则正文缓存为优先边界；MongoDB、控制台、多租户自定义工作流和安装 `.github` / `CLAUDE.md` / `AGENTS.md` 副本只能作为后续远景或经确认范围。
- **WorkspaceDataAbsorptionScopeGate**：当任务是扫描 data、吸纳最新问题、检查仍需吸纳清单或从运行时台账提需求时，必须扫描工作区 `.devcodex/*/data/` 的所有项目命名空间和 workspace 台账，输出命中/跳过/归属证据。
- **FlowchartNodeExplanationGate**：需求、方案、文档站或维护者页新增正式 Mermaid 流程图时，每个非终止节点必须有中文节点说明；若只是临时草图或低风险内部注记，写 `N/A + skipReason`。
- **DocsSiteVisualAcceptanceGate**：官网/文档站/技术站改动命中视觉或交互时，CP2/TestRoute 必须覆盖主题集成、真实点击、异步动效、减弱动态、代码 token 对比度、终端 demo 范围、TOC inline code 和辅助导航层级。
- **OmissionOnlyReviewGate**：用户要求只审遗漏或只列仍需吸纳项时，不得把已吸纳、已关闭、重复项或明确无必要项重新放回最终清单；报告需保留排除理由。
- **MethodLevelLeakPressureProbe**：资源泄漏修复、公开方法、adapter/SDK 或连接/监听/定时器/worker/cache 风险命中时，除常规测试外评估方法级重复调用或生命周期压测；不触发写 `N/A + skipReason`。
- **V2FormalSolutionPackage**：DevCodex v2 一期正式规划必须形成 CP1/CP2 方案包，覆盖架构、数据模型、MCP API contract、最小 instruction return、可见性、缓存签名回滚、Codex-only 验证、Registry/Marketplace、私有维护站和 Mermaid 节点流程。
