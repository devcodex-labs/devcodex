...
- **CrossProjectLearnedGuards**：修复涉及接入状态误判、人工复核、翻译/正式文档边界、prompt/Hook/MCP 契约、验证范围不足或过度、未真实执行验证、adapter/provider benchmark 归因（`AdapterBenchmarkAttribution`）、产品需求来源、本机执行配置、人工证据留存、相邻范围扩展、包名/发布名、性能第一、公开模块、提交边界、兼容契约（`CompatibilityAndContractAuthorityGate`）、UI 真相源冲突、公开文档版本边界、集合关系命名（`CollectionRelationIdNamingGate`）、验证产物语言、复审维度重复、使用者文档心智负担、公开用户文档混入维护者 checklist、文档消费者同步、产物链接重复展示、前端真实预览网络探针、最终回复范围漂移、数据库记录迁移、guard/policy 绕过、审查 finding 反证矩阵、带副作用兼容文档、可执行示例、一次性脚本归属、验证命令副作用、package/adapter 确认前证据、需求确认前缺口或多阶段关闭时，必须分别执行对应 `CrossProjectLearnedGuards`；未触发时写 `N/A + skipReason`。
