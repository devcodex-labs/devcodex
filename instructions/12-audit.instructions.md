...
| R2~Rn | 每轮聚焦剩余未解决问题；**R2 起若发现新问题，必须触发自我审视**（见 `audit-common §自我审视机制`）|
| 收敛前门禁 | **CRS ✅**（见下方说明）|
| 收敛条件 | **定向审查 = 至少 3 轮，且连续 2 轮无新发现；全面体检 = 至少 3 轮，且连续 3 轮无新发现**，同时所有 🔴 已解决 + 🟡 级问题已处理或标注 N/A |

- 未收敛时自动进入下一轮，**不询问用户**
- 🔴 **CRS 收敛门禁**：宣告收敛前，必须在 `instructions/` 和 `skills/` 全库 grep 本次审查涉及的核心关键词，确认所有引用相同概念的文件均已纳入审查范围并完成 G3 检查；CRS 发现未审查文件时，**收敛计数重置为 0**
- 🔴 "至少 3 轮"是硬性下限：即使前几轮已无新发现，仍须完成最少 3 轮后才可声明收敛
- 禁止提前收敛：CRS 未完成或不达到最少轮次时不得输出"已收敛"结论
- 最少轮次规则对所有 audit 子类型一律适用（规范文件/技术方案/需求文档/项目工程/报告/通用文档）
