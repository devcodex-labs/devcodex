...
> 🔴 **跨会话恢复硬约束**：当上下文来自会话摘要（summary/checkpoint）时，**不免除预检查**。摘要内容不构成 Profile 已加载的证明，也不构成记忆已读取的证明。每条新用户消息到达后，必须重新执行完整 PC0~PC4，**第一批 tool call 必须包含 Profile 读取 + 记忆读取（Agent SUMMARY.md + 今日/昨日日记）**，不得直接进入任务执行。
