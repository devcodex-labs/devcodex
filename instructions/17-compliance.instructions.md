...
> 🔴 **入口无例外原则**：**每一条用户消息到达 = 无条件触发完整入口流程**，与消息长短、复杂度、工作流类型无关。入口流程是物理门禁，不是"视任务需要选择性执行"的步骤。
>
> **入口流程操作性定义（严格按序）**：
> 1. 第一批 tool call：Profile 读取 + 记忆读取（SUMMARY + 今日/昨日日记）
> 2. 回复的**第一行输出**：预检查块 PC0~PC4（不得在预检查块之前输出任何分析、方案或代码内容）
> 3. 预检查完成后：进入意图识别 → 工作流路由 → 执行
