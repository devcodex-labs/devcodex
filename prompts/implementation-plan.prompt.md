...
| ExpertOwnerSkillGate | 对照专家 Owner Skill 门禁 | 命中专业语义时 ownerSkill、triggerReason、requiredFields、validationRoute、skipReason 完整；17 个 Skill 的关键字段与 V85/targeted probe 覆盖正反样例；增长 / 商业未触发写 N/A |
...
- [ ] ExpertOwnerSkillGate 已完成或记录 `N/A + skipReason`（命中产品策略、开发者体验、UX、前端、后端领域、SRE、API 契约、外部集成、平台生态、AI Agent、数据、安全、质量、设计系统、无障碍国际化、增长分析或商业模型专业语义时，必须给出 ownerSkill、requiredFields、validationRoute 和 V85 证据；增长 / 商业为 P3 条件触发）
