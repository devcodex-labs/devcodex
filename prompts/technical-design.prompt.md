...
> 推荐格式：`模块 | 文件 | 变更职责 | 依赖`（4 列）。有"文件"列便于 CP3 直接映射到实施任务。
...
> 按子节组织：① Schema/Model 变更（字段增删、enum 变更）② 类型定义（新增 interface/type）③ 入口行为变更表（新增参数、新错误码）④ 异步处理器路由（Webhook/消息队列新增分支）⑤ 接口调用示例（对外入口有参数变更时必填：每个受影响的接口补充请求体示例 + 关键场景响应，帮助评审方快速理解 API 契约，无需阅读代码）。  
> ⚠️ 新增错误码须同步规划多语言条目（`ctx.dsl.error.throw(KEY)` 无 i18n 条目时返回裸 key 字符串给客户端）：core 通用错误加 `config/dsl/core/`，模块专属错误新建 `config/dsl/<module>/` 并注册到 `dslDirs`。
...
> 包含：类型检查（tsc --noEmit）、单元测试（核心计算/校验函数）、集成/联调（对外接口 happy path + edge case）、回归（现有功能未被破坏）。

| 类型 | 工具 | 覆盖目标 |
|------|------|---------|
| 类型检查 | tsc --noEmit | 新增类型定义无 TS 错误 |
| 单元测试 | | |
| 集成测试 | | |
| 回归 | | |
