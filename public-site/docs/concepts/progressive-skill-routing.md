...
目录可能有多页，选择时需要看完当前范围内的候选。读取资料中的 Skill 名称不等于激活该 Skill；只有当前任务选择、对应正文和依赖的成功读取才计入执行阶段。收尾材料在进入收尾阶段时读取，入口不因未来可能需要就提前加载。

升级后，新连接使用新运行代次及其对应的 Skill 内容；已打开的连接可以继续原代。不能仅凭已安装版本号判断当前连接已经加载新代码。
