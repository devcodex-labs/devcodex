...
function buildGovernanceIntakeChecks(ctx) {
  ...
  function checkV44() {
    const probes = [
      ...
      { file: 'skills/test-router/SKILL.md', needles: ['regressionChecks', '历史能力、必跑验证、对应批次和失败回滚点', 'strict-descendant-same-scope', 'parentRootReceiptDigest + parentTerminalDigest'] },
      { file: 'scripts/run-validation.js', needles: ['assessAutoRootRollover', 'assertRootReplacementSafe', 'auto-root-rollover-plan-only', 'auto-root-rollover-authorized', 'candidateBinding(candidate)'] },
      ...
      { file: 'scripts/test-validation-budget-control.js', needles: ['validation-budget-root-rollover-session', 'live-root-replacement', 'server-owned store must reject direct live root replacement', 'root-rollover-widened', 'root-rollover-completed-child', 'auto-root-rollover-plan-only', 'auto-root-rollover-authorized', 'rootRolloverReason'] },
