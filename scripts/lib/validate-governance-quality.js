...
function buildGovernanceQualityChecks(ctx) {
  ...
  function checkV56() {
    const probes = [
      ...
      { file: 'instructions/10-dev.instructions.md', needles: ['前置平台工程判断', '包边界验证串行化', '消费者依赖树优先探针', '接入状态口径拆分', '只能清理具备本轮 receipt', '来源不明的 dirty 只能报告、隔离或从候选中排除', '禁止 reset/stash/delete/overwrite'] },
      { file: 'instructions/11-fix.instructions.md', needles: ['前置平台工程判断', 'npm ls <关键依赖>', '只能清理具备本轮 receipt', '来源不明的 dirty 只能报告、隔离或从候选中排除', '禁止 reset/stash/delete/overwrite'] },
      ...
      { file: 'skills/release-verification/SKILL.md', needles: ['发布型 Profile', '单独串行执行', '盘点无关 dirty', 'ownership receipt', '禁止自动删除、reset、checkout、stash 或覆盖'] },
