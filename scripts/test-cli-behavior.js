...
function testRuntimeStatusAndPruneAreBounded() {
  ...
  assert.strictEqual(maintenancePreview.payload.runtimeGenerations.schemaVersion, 'BoundedMaintenancePreviewV2')
  assert(['generation-scan', 'manifest-ready'].includes(maintenancePreview.payload.runtimeGenerations.phase))
  assert(Number.isInteger(maintenancePreview.payload.runtimeGenerations.progress.scanned))
  assert(Number.isInteger(maintenancePreview.payload.runtimeGenerations.progress.total))
  assert.strictEqual(maintenancePreview.payload.runtimeGenerations.budget.maxGenerations, 16)
  assert.ok(maintenancePreview.payload.runtimeGenerations.manifest)
  assert.strictEqual(maintenancePreview.payload.runtimeGenerations.deletedFiles, 0)
  assert.strictEqual(maintenancePreview.payload.runtimeGenerations.deletedDirectories, 0)
...
function testGlobalOnlyHostSelectorsFailClosed() {
  ...
  assert.match(uninstall.nextStep, /devcodex uninstall --dry-run/)
  const isolatedHome = path.join(root, 'isolated-home')
  const bareResult = runCliResult(['uninstall', '--json', '--home', isolatedHome], root)
  assert.strictEqual(bareResult.status, 0, bareResult.stderr || bareResult.stdout)
  const bareUninstall = JSON.parse(bareResult.stdout)
  assert.strictEqual(bareUninstall.ok, true)
  assert.strictEqual(bareUninstall.payload.operation, 'remove')
  assert.strictEqual(bareUninstall.payload.dryRun, true)
  assert.strictEqual(bareUninstall.payload.status, 'already-absent')
