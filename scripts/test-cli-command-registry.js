...
assert.strictEqual(runCliCommand({ cmd: 'uninstall', argv: [], registry, runMigrateLayout: migrate, process: fakeProcess, c, console: logger }), 'global-adapters-remove')
assert.deepStrictEqual(calls.pop(), ['cmdGlobalAdapters', ['remove']])
assert.strictEqual(runCliCommand({ cmd: 'uninstall', argv: ['--apply', '--json'], registry, runMigrateLayout: migrate, process: fakeProcess, c, console: logger }), 'global-adapters-remove')
assert.deepStrictEqual(calls.pop(), ['cmdGlobalAdapters', ['remove', '--apply', '--json']])
...
assert.strictEqual(runCliCommand({ cmd: 'tmp', argv: ['status', '--json'], registry, runMigrateLayout: migrate, process: fakeProcess, c, console: logger }), 'tmp')
assert.deepStrictEqual(calls.pop(), ['cmdTemp', ['status', '--json']])
assert.strictEqual(runCliCommand({ cmd: 'governance', argv: ['ledger', 'plan', '--kind', 'GR', '--json'], registry, runMigrateLayout: migrate, process: fakeProcess, c, console: logger }), 'governance')
assert.deepStrictEqual(calls.pop(), ['cmdGovernance', ['ledger', 'plan', '--kind', 'GR', '--json']])
fakeProcess.exitCode = 0
...
assert.strictEqual(runCliCommand({ cmd: 'help', argv: ['tmp'], registry, runMigrateLayout: migrate, process: fakeProcess, c, console: logger }), 'help')
assert.deepStrictEqual(calls.pop(), ['cmdHelp', ['tmp']])
