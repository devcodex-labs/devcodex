...
assert.match(workflow, /workflow_dispatch:[\s\S]*publish-qualified/)
...
assert.ok(workflow.indexOf('exact-release-artifact.js create') < workflow.indexOf('test-global-install-smoke.js --tarball'))
assert.ok(workflow.indexOf('test-global-install-smoke.js --tarball') < workflow.indexOf('actions/upload-artifact@v4'))
assert.match(workflow, /if: always\(\) && github\.event_name == 'workflow_dispatch' && inputs\.mode == 'publish-qualified'/)
assert.match(workflow, /qualified-release-\$\{\{ inputs\.release_tag \}\}-\$\{\{ inputs\.qualification_run_id \}\}/)
assert.match(workflow, /run-id: \$\{\{ inputs\.qualification_run_id \}\}/)
assert.match(workflow, /RELEASE_TAG.*inputs\.release_tag[\s\S]*Release tag .* does not match package version/)
assert.match(workflow, /publish-qualified requires numeric qualification run identity/)
assert.match(workflow, /PublishedArtifactReceiptV1/)
assert.match(workflow, /published-release-\$\{\{ inputs\.release_tag \}\}/)
assert.match(workflow, /run-id: \$\{\{ inputs\.publish_run_id \}\}/)
assert.match(workflow, /finalize-only requires numeric publish run identity/)
