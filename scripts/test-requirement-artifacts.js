...
try {
  ...
  write(goodBugProgress, `${qualifiedTemplateText(slot('implementation-progress'), goodBugProgress, 'fix')}\n> 当前 CP：执行中\n\n## 进度总览\n\n## 支撑产物状态\n\n**本轮验证结果**：\n\n## 阻塞与恢复\n\n## 下一步\n\n## 变更记录\n`)
  ...
  const staleBugRoot = path.join(bugsRoot, 'stale-status-bug')
  const staleBugOverview = path.join(staleBugRoot, '00-问题概况.md')
  const staleBugCp1 = path.join(staleBugRoot, '01-问题确认.md')
  const staleBugCp2 = path.join(staleBugRoot, '02-修复方案.md')
  const staleBugPlan = path.join(staleBugRoot, '04-实施计划.md')
  const staleBugProgress = path.join(staleBugRoot, '05-实施进度.md')
  write(staleBugOverview, `${qualifiedTemplateText(slot('bug-overview'), staleBugOverview, 'fix')}\n> **状态**: CP1 已确认，待 CP2\n`)
  write(staleBugCp1, `${qualifiedTemplateText(slot('bug-cp1'), staleBugCp1, 'fix')}\n> **状态**: CP1 已确认，待 CP2\n`)
  write(staleBugCp2, `${qualifiedTemplateText(slot('bug-cp2'), staleBugCp2, 'fix')}\n> **状态**: CP2 候选，等待用户确认\n`)
  write(staleBugPlan, qualifiedTemplateText(slot('implementation-plan'), staleBugPlan, 'fix'))
  write(staleBugProgress, `${qualifiedTemplateText(slot('implementation-progress'), staleBugProgress, 'fix')}\n> 当前 CP：执行中\n\n## 进度总览\n\n## 支撑产物状态\n\n**本轮验证结果**：\n\n## 阻塞与恢复\n\n## 下一步\n\n## 变更记录\n`)
  ...
  assert(bugResult.checkedDirs.includes('stale-status-bug'))
  ...
  assert(bugResult.issues.some(item => item.includes('stale-status-bug/00-问题概况.md') && item.includes('stale CP2 status projection')))
