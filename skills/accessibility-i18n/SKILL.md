...
| `InteractiveSemanticProbe` | 每个交互对象必须验证 role、accessible name、可聚焦性、键盘动作和关闭后的焦点恢复 | semanticInteractionMatrix、runtime evidence |
...
5. 对链接、按钮、菜单、对话框、可展开控件和自定义交互执行 `InteractiveSemanticProbe`：核对 role、accessible name、focusability、Enter/Space/Escape 行为和 focus recovery。
6. 选择验证路线：源码语义、浏览器键盘/辅助技术 smoke、文档预览、测试或人工证据；截图只能证明视觉状态，不能替代交互语义证据。
7. 在技术方案、TestRoute 或报告中写明未触发场景的 `N/A + skipReason`。
...
| interactiveSemantics | role、accessible name、focusability、Enter/Space/Escape、focus recovery |
| focusRecovery | Escape/关闭/失败恢复后焦点回到触发对象或下一合法任务点的运行态证据 |
...
| 用截图或 DOM 存在证明控件可访问 | 执行 InteractiveSemanticProbe；视觉存在不等于键盘可操作或辅助技术可理解 |
