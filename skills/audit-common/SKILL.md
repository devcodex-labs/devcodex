...
- 🔴 **连续 3 轮有效零发现**不可跳过：无论之前已运行多少轮，必须连续 3 轮均无新发现，且每轮覆盖增量和维度增量都合格，方可声明收敛
- CRS 发现新文件时，零发现计数**重置为 0**
- 禁止提前收敛：CRS 未完成、未达到 3 轮零发现，或 `ReviewCoverageDelta` / `ReviewDimensionDeltaGate` 不合格时不得输出"已收敛"结论
