...
| 批次 | 维度 | 说明 |
|------|------|------|
| 🔴 第一批 | D1·D2·D3·D4·D5·D7·D9·D10·D11·D12·D16·D17·D21·D22 | 强制，立即标记 |
...
```
批次 1（🔴 核心维度）：D1·D2·D3·D4·D5 → 输出中间报告 → 写入记忆
批次 2（🔴 补充维度）：D7·D9·D10·D11·D12 → 追加报告 → 更新记忆
批次 3（🟡 建议维度）：D6·D8·D16·D17·D18 → 追加报告 → 更新记忆
批次 4（💡 改进维度）：D13·D14·D15·D19·D20·D21 → 追加报告 → 更新记忆
CRS 收敛门禁：最后一批结束后、PCV 前执行全库关键词扫描（见 audit-common §关联文件发现）
PCV（收敛后汇总验证）：CRS ✅ 后执行 PCV（见 audit-common §收敛后汇总验证）→ 最终报告
```
