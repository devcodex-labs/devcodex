...
```
批次 1（🔴 核心维度）：D1·D2·D3·D4·D5 → 输出发现 → 即发即修（有发现则立即 self-fix）→ 写入记忆
批次 2（🔴 补充维度）：D7·D9·D10·D11·D12 → 输出发现 → 即发即修 → 更新记忆
批次 3（🟡 建议维度）：D6·D8·D16·D17·D18 → 输出发现 → 即发即修 → 更新记忆
批次 4（💡 改进维度）：D13·D14·D15·D19·D20·D21 → 输出发现 → 即发即修 → 更新记忆
...
> ℹ️ **即发即修的含义**：每批次发现问题后，在开始下一批次前先完成 self-fix 修复；下一批次审查的是修复后的最新状态，而非原始状态。
