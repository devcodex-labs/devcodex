...
| CP3 | 实施计划确认 | 🔴 必须 | 条件触发 | 确认实施计划后开始逐文件执行 |
...
7. **产物文件前置创建**：输出 CP 确认请求前，对应产物文件必须已写入磁盘（CP1 → `01-需求概述.md` + `<需求>/.memory/sessions.md`（需求级记忆，🔴 强制创建）；CP2 → `02-技术方案.md`（有架构/接口/设计决策时，否则跳过））
