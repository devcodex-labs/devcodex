...
**数据吸纳扩展守门（F-33）**：来源于 `data/*.md`、用户纠偏或同类项目的剩余吸纳项，只有通过价值复核且能落入 CP/TestRoute/审查/validate 的，才进入规范源；其中产品需求、执行配置、人工证据、相邻范围、包名、性能基线、公开模块、v2 MCP-first 路线、全工作区 data 扫描、流程图节点解释、文档站视觉验收、遗漏专审、公开用户文档维护边界、最终回复 active 范围、方法级泄漏压测和 v2 正式方案包必须分别执行对应 Gate。
