...
**最小实现守门（F-27）**：执行阶段必须按 CP1/CP2/CP3 的 `ImplementationComplexityPreference` 与复杂度预算落地；用户未要求复杂化时默认 `simple`，只做满足验收的局部最小实现；禁止无计划新增抽象、通用配置、预留扩展点或未确认防御分支。若确需超出预算，先暂停并回 CP2/CP3。
