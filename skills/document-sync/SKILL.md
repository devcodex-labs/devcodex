...
- 用户可见输出契约变化时，把 `ArtifactDeliveryManifestV1 → UserFacingArtifactSetV1 → DevCodexVisibleEnvelopeV1 → LinkCapabilityDecisionV1` 同步到 instructions、report/memory/compliance/TestRoute、precheck/progress/delivery/report prompts、visible Hook、README、website、Profile、plugin/portfolio、package 与 validate probes；历史归档可保留旧表述，但当前消费者不得继续按宿主名强制绝对路径或把 internal-only 文件默认列给用户。
