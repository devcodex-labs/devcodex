...
- 前端 UI / 交互体验规范是否同步到 `FrontendExperienceQualityGate`、TestRoute、报告模板、README/website 与 validate；涉及 Figma/截图/既有页面还原、真实 preview、状态回归、生产资产或运行时 i18n 时，还要同步 `FigmaHighFidelityRestorationGate`、`ScopedVisualChangeGate`、`InstalledPluginVisualVerificationGate`、`ActualPreviewChainAndMockFallbackGate`、`UIStateScopeRegressionGate`、`FigmaProductionAssetBudgetGate` 与 `RuntimeI18nArtifactVerificationGate`；视觉或交互证据不得只停留在口头说明
