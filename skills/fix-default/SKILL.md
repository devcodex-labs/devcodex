...
- 修复范围不得超出问题边界（禁止顺手重构）
- 最小实现守门：CP1 问题确认必须给出 `ImplementationComplexityPreference`，默认 `simple`；修复只解决已确认根因和影响范围，禁止无计划新增抽象、通用配置、预留扩展点或未确认防御分支；确需超出范围或升级到 `balanced/robust` 时回 CP2/CP3 并等待确认
