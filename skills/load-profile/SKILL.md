...
- `ProfileSectionSelectionGate`：文件已进入 plan.selected 后，可调用 `profile_load({ files, sectionSelectors })` 按完整 Markdown section 读取。selector 必须给出 `file / headingQueries / requiredQueries`，并显式选择 `includePreamble / includeDescendants / maxBytes`；请求顺序就是返回顺序，禁止按字符中途截断。
- `ProfileSectionLoadReceiptV1` 必须记录 full/selected bytes、source digest、matched/missing/ambiguous/deferred、completion 与 fallbackReason。required query 缺失或歧义、低置信、parser 不支持、required section 超预算时自动 `fallback-full`；optional section 预算不足只能 `partial`，调用方补读 deferred 或整文件前不得把 Context receipt 推进到 relevant-complete。
