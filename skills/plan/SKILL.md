...
- **涉及文件修改** — 遵守 [C01](../../instructions/01-common.instructions.md)（需先输出计划等待确认）
- **无 CP 强制要求** — other 路由不强制 CP1→CP2→CP3，但多步复杂任务时建议分步确认
- **不跳过合规检查** — 执行完成后仍需经过 [`compliance`](../compliance/SKILL.md) Skill（[C05](../../instructions/01-common.instructions.md)）
