...
- **EnforcementHonesty（R5）**：完成态报告或最终回复须含「拦截与流程诚实摘要」表（host / hard 事件 / Stop 决策 / processGaps）；模板见 `compliance` Skill；枚举含 `pr1-skipped`、`cp2-unconfirmed-write`、`entry-check` 等
