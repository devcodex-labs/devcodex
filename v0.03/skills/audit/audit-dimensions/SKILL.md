...
| 5 | `pre-message.hook.md` 步骤列表与 `devcodex.agent.md` 前置检查流程是否对齐 |
...
| 3 | `post-session.hook.md` 引用的 report 模板是否覆盖所有工作流 |
...
| 3 | `pre-message.hook.md` 的步骤顺序是否与 `01-common.instructions.md` 的节点顺序一致 |
...
| # | 检查内容 |
|:-:|---------|
| 1 | `MIGRATION.md` 中的 v4→v5 映射表是否完整（所有 v4 specs 都有对应 v5 组件）|
