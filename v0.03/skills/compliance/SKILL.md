...
读取由 `load-profile` Skill 注入的 ENV_MODE：
...
> ⛔ **S01~S06 安全底线不受 ENV_MODE 影响**，无论 dev/prod 均强制执行。
>
...
>
> ℹ️ **预检查（PC1~PC3）仅在 dev 模式启用**，收到用户消息后立即执行，详见 `17-compliance.instructions.md §预检查`。
