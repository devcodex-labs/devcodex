...
- README / 用户使用文档默认通过 `readme-authoring` 收口用户 / 使用者优先写作，完成后再用 `audit-readme` 做专项 review；正式用户文档同步执行 `UserPerspectiveDocsGate`、`PublicUserDocsMaintainerBoundaryGate` 与 `DocsConsumerSweep`，检查第一次成功、常见任务、字段/参数/状态/错误解释、排错恢复、低心智负担，公开用户路径不混入维护者 checklist / 内部同步清单 / 台账状态，以及 README / website / Profile / examples / templates / validate / 代码消费点同步
