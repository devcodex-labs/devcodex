...
> v1.15.2 的 `publishConfig` 指向 GitHub Packages restricted 通道；`release:dry-run:npmjs` / `release:dry-run:github` 继续保留为多目标发布前诊断工具，但当前版本只授权和验收 GitHub Packages。真实发布仍必须完成 package ownership / publisher / auth topology、目标提交远端 CI 与目标 registry 后验收；dry-run 通过不能替代真实发布证据。
