...
2. 按每条 `command` 执行；源码候选优先 `devcodex global-adapters apply`，或 `npm install -g .` / `npm install -g ./vextjs-devcodex-1.15.3.tgz`；发布后缺失时使用 `npm install -g devcodex`、升级时使用 `npm update -g devcodex`。
