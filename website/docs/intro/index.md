...
| `npm pack` + `npm install -g ./devcodex-*.tgz` | 预发冒烟 tarball |
...
| Skills 86 | `plugin.json` + 磁盘 SKILL.md | `npm run test:docs-surface-inventory` | 不要求 |
...
> Skill 规模锚点：86 个 Skills；扁平一级 Skill（86 个）。
