...
| Skills | 43 个按需触发的工作流技能，覆盖完整开发生命周期、规范治理、`readme-authoring` / `audit-readme` README 专项能力，以及 `execution-contract` / `test-router` / `release-verification` / `host-contract-verification` / `source-consumer-sync` 支撑能力 |
| Prompts | CP 节点输出模板 |
