...
Auto v1.1（`@devcodex-auto` 或明确自然语言 auto 授权）并不是"去掉约束"。它只在 hook-enforced 宿主里，对治理文件、文档、`.devcodex/` 产物、README 与 auto 专属回归脚本等白名单路径自动推进；模糊提及、询问 auto 规则或普通“继续”不算授权；非白名单源码路径默认回确认模式，安全底线始终有效。
...
| `@devcodex-auto` / 明确自然语言 auto 授权 | 熟悉流程后的白名单路径快速迭代；非白名单源码路径回确认模式 |
