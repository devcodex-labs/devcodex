...
全自动模式（`@devcodex-auto`）并不是"去掉约束"，而是"由 AI 自行完成确认判断"——适用于用户已熟悉流程、风险可控的场景，但安全底线始终有效。

---

## 合规检查：AI 对自身行为的自检

合规检查（FC/SC/RC）是 **AI 在开发工作流执行阶段**对自身行为的强制验证，不是对用户代码的生产合规检查。
...
合规检查本质是一套**工作流执行质量保证**机制，保证每次 AI 的工作过程是完整的、可追溯的。
