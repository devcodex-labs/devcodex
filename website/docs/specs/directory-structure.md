...
| 第三层 | **Skills** | 按需触发的工作流能力入口 | dev / fix / audit / analyze / self-fix / plan / resume / chat，以及 execution-contract / test-router / release-verification 等支撑能力 |
...
`spec-governance` 是规范治理专用 Skill，负责记录类意图识别、RecordRouter 台账分流，以及 SCV（Spec Change Verification）规范变更验证。`execution-contract`、`test-router`、`release-verification` 是支撑型 Skill，用于长流程边界、验证路线和正式发布前检查，不新增工作流分支。
