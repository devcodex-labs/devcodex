...
| 选择 `@devcodex-auto` 后执行登录任务 | 跳过 CP 直接执行，但安全底线仍触发 |
...
| 场景 | 预期行为 |
|------|---------|
| 在 `@devcodex-auto` 中修改文件名 | 跳过 CP，直接执行 |
| 在 `@devcodex-auto` 中删除所有 `.tmp` 文件 | C01 触发，仍等待确认 |
| `@devcodex-auto` 任务中 lint 失败 | 自动重试 ≤2 次，仍失败标 ⚠️ 停止 |
