...
### ENV_MODE 对预检查的影响（1.0.0 阶段）

| ENV_MODE | 预检查行为 |
|----------|----------|
| `dev` | 输出预检查状态块（`1.0.0` 阶段为 PC0~PC3，当前为 PC0~PC7）；全量合规检查 FC+SC+RC+T |
| `prod`（默认）| `1.0.0` 阶段不输出预检查状态块；不执行合规检查 |

> 历史说明：上述表格描述 v0.03 / `1.0.0` 阶段规则；当前 `17-compliance.instructions.md` 已演进为全模式 PC0~PC7，`prod` 仅不执行后置合规检查。
