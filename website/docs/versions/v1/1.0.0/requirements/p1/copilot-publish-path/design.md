...
3. **MCP / 现有包分发路径**：作为当前更合理的主路线，分为短期继续包分发、长期补齐 MCP 能力两步。

结论先行：**当前 DevCodex 不适合直接定义为 GitHub Marketplace App；更合理的路线是维持现有包分发作为短期发布路径，并把 MCP 化作为中长期主方案。**
...
采用 **“短期包分发，长期 MCP 化”** 的双阶段路线：

1. **短期主发布路径**：继续以 npm / GitHub Packages / 仓库文档站分发 DevCodex。
2. **中长期目标路径**：把 DevCodex 演进为真正的 MCP 服务，再接入 GitHub 当前推荐生态。
