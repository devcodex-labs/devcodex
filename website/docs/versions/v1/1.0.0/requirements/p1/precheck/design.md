...
预检查的**语义规则**由 `17-compliance.instructions.md` 的 PC0~PC4、`01-common.instructions.md` 的 Profile / 路由规则，以及 `intent` / `load-profile` Skill 共同定义；在支持 Hooks 的宿主中，触发时机可由 `.github/hooks/` 提供硬门禁。

---

## 核心设计

按当前 Instructions-First + Hook-First/Fallback 结构，预检查主链为：
...
5. **输出 PC0~PC4 状态块** — 由 `17-compliance.instructions.md` 定义预检查输出格式与 dev 模式强制性
...
| 文件 | 角色 |
|------|------|
| `instructions/17-compliance.instructions.md` | PC0~PC4 预检查入口与输出格式 |
