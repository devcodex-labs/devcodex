...
> ⚠️ 历史快照说明：本页保留 `1.0.0` 阶段的 Agent 需求。当时文档中的 Free/Pro 层级、语言规划和 Skills 数量是历史口径；当前版本以 `plugin.json`、`README.md` 与 `skills/token-check/SKILL.md` 为准，当前所有工作流全量开放。
...
```
agents/
├── devcodex.agent.md       # 确认模式（默认入口）
└── devcodex-auto.agent.md  # 全自动模式入口
```

## 功能需求

### 基本信息
- Agent ID: `devcodex` / `devcodex-auto`
...
### Skills 声明
- 通过 Agent 文件 HTML 注释声明所有 Skills
- 核心 Skills 同时注册在 `plugin.json`

## 验收标准
- [ ] 双 Agent 可被 VS Code Copilot 加载
- [ ] 意图路由覆盖 8 种工作流
- [ ] 违规质疑特殊路由生效
- [ ] 全局约束注入完整
