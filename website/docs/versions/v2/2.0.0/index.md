...
v2.0.0 一期默认路线是 **Intent-Gated Hosted Spec MCP**：先做 Codex-only 可验证 MVP，通过宿主意图识别后从托管 MCP 获取规范片段、私有可追踪 docs 与验证证据。

正式方案包入口：[v2 一期正式方案包](./formal-solution-package)

一期边界：

- 不安装 `.github` / `CLAUDE.md` / `AGENTS.md` 规则副本作为默认分发方式。
- 不本地持久化缓存规则正文；只允许短期会话态、trace id 或可审计引用。
- 不把 MongoDB、控制台、多租户自定义工作流作为一期默认范围。
- 不在正式 CP1/CP2 方案包冻结前启动平台化数据层或控制台实现。
- 先验证 Codex-only 意图门禁、MCP 获取、私有 docs 可追溯、fallback 与安全边界。

---

## 历史平台化目标（非一期默认）

以下内容属于历史远景假设。未通过一期正式方案包前，不得作为 v2.0.0 一期默认实施范围。

v2.0.0 远景曾设想作为 DevCodex 的商业化与平台化起点，引入云端存储层，支持：
...
| 项目 | v1.0.0 | v2.0.0 一期 | v2.0.0 远景 |
|------|--------|-------------|------------|
| 收费策略 | 免费 | 暂不作为一期阻断项 | 商业化分层（Free / Pro / Enterprise） |
| 产物存储 | 本地 Markdown / 文件系统 | Hosted Spec MCP + 私有可追踪 docs；不本地持久化缓存规则正文 | MongoDB + 平台化数据层 |
| 目标 | 规范冻结与开发准备 | Codex-only MCP 获取链路和安全 fallback 验证 | 平台化、租户化、商业化 |

---

## 架构决策

> 本节 D-001~D-004 是历史规划记录。与“一期当前路线（MCP-first）”冲突时，以一期路线为准；任何平台化、MongoDB、控制台或多租户自定义工作流实现都必须先回到 CP1/CP2 重新确认。
...
## v2.0.0 开发准备事项（一期方案包冻结后开始）

- [ ] 冻结一期 CP1/CP2 方案包：Intent-Gated Hosted Spec MCP、Codex-only MVP、私有可追踪 docs、fallback、无本地规则正文缓存
- [ ] 明确 `.github` / `CLAUDE.md` / `AGENTS.md` 规则副本不作为一期默认安装范围
- [ ] 明确 MongoDB、控制台、多租户自定义工作流为远景或后续确认范围
...
- [ ] 重审平台化数据层是否仍需要 MongoDB 托管方案（Atlas / 自建）
- [ ] 重审控制台、多租户自定义工作流和本地到云端迁移工具是否进入后续版本
...
| v2.0.0 alpha | Codex-only Intent-Gated Hosted Spec MCP MVP |
| v2.0.0 beta | MCP 获取链路、私有 docs 追踪、fallback 与验证证据稳定 |
| v2.0.0 正式 | 一期边界内发布；MongoDB/控制台/多租户自定义工作流需独立确认 |
